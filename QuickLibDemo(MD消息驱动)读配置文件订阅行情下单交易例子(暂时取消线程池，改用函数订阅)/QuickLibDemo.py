...
def main():
    ...
    market.AddPeriod('zn1705',YT_M10)   #添加M3周期（未在Subcribe、Subcribe1~Subcribe8函数中指定的周期，可以在本函数补充该品种周期，可多次调用函数设置同时保存多个周期）
    ...
    while(1):  #死循环
        ...
        for i in range(0, 3):
               tempclose=market.GetPeriodData('ag1612',YT_M1,YT_CLOSE,i)
               if tempclose>0:
                  print tempclose
        print u'-----------end----------------'
